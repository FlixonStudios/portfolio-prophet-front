...
function Details({ auth, watchlist, addToWatchlist, removeFromWatchList }) {
    ...
    useEffect(() => {
        async function getStock(region = 'SG') {
            let { data } = await axios.get(
                `/api/stocks/getStock?symbol=${symbol}&region=${region}`,
            )
            setStockDetail(data)
    ...
    if (forecastRecord) {
        data = forecastRecord.map((element) => {
            const dateConvert = new Date(element['date']).toLocaleDateString(
                'en-GB',
    ...
    return (
        <div className={`section details ${auth && 'dash-card-block'}`}>
            <Row className="no-gutters justify-content-center">
                <Col
                    className={`d-flex align-items-center mb-5 ${
                        auth ? 'col-12 col-sm-8 col-md-6' : 'col-11'
                    }`}
                >
                    <h2>
                        {stockDetail.quoteType.shortName} ({stockDetail.symbol})
                    </h2>
                </Col>
                {auth && (
                    <Col
                    ...
                    >
                        <button className="btn btn-primary mb-3 mb-lg-0 mr-0 mr-lg-3">
                            Add to Portfolio
                        </button>
                        {foundStock ? (
                            <button
                                className="btn btn-secondary"
                                onClick={() => resetButton(stockDetail.id)}
                            >
            ...
            <Row className="no-gutters justify-content-center">
                <Col
                    className={`${
                        auth ? 'col-12 col-xl-5' : 'col-11 col-xl-5'
                    }`}
                >
